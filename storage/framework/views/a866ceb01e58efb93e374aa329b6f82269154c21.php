...
<form class="content container-fluid my-3" action="<?php echo e(URL::route('sales.update', $sale->id)); ?>" method='post'>
		<div class="card">
		...
		<div class="card-body p-3">
			...
			<div class="col-md-6 col-lg-4 col-xl-3">
				<div class="form-group">
					...
					<input type="text" placeholder="" name="order_number" value="<?php echo e($sale->order_number); ?>" readonly class="form-control">
			...
			<div class="col-md-6 col-lg-4 col-xl-3">
				<div class="form-group">
					...
					<input type="text" autocomplete="off" class="datepick form-control" required name="order_date" value="<?php echo e($sale->order_date); ?>">
					<span style="color:red"><?php echo e($errors->first('order_date')); ?> </span>
				</div>
			</div>
			<input type="hidden" class="sale_id" value="<?php echo e($sale->id); ?>">
			<div class="col-md-12">
...
<table id="purchase_table" class="order-table controls table table-light table-bordered table-striped table-hover">
	...
	<tbody>
    <?php $__currentLoopData = $sale->items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $va1=>$key1): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
	<tr class="entry">
	   <td>
	   ...
	   <select id="role_type" name="role[]" class="form-control" onchange="getUsers(this);" required >
			...
			<?php if(Auth::user()->isAdmin()): ?>
				<option value="2" <?php if($key1->user->role == 2): ?> selected <?php endif; ?> >Distributor</option>
			<?php endif; ?>
			<option value="3" <?php if($key1->user->role == 3): ?> selected <?php endif; ?>>Dealer</option>      
		</select>
		...
		<td><select id="user" name="user_id[]" class="form-control user_id" required>
                ...
                <?php $__currentLoopData = $key1->allusers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $va=>$key): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                     <option value="<?php echo e($key->id); ?>" <?php if($key1->user->id == $key->id): ?> selected <?php endif; ?> ><?php echo e($key->name); ?></option>
		...
		<select id="new_brand" name="company_id[]" class="form-control company_list" onchange="getProduct(this);" required>
            ...
            <?php $__currentLoopData = $company; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $va=>$key): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($key->id); ?>"  <?php if($key1->product->company_id == $key->id): ?> selected <?php endif; ?>><?php echo e($key->name); ?></option>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
		    </select>
		</td>
		<td>
            ...
            <option value="">Select Product</option>
                <?php $__currentLoopData = $key1->allproducts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $va=>$key): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                     <option value="<?php echo e($key->id); ?>" <?php if($key1->product_id == $key->id): ?> selected <?php endif; ?> ><?php echo e($key->name); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
			</select>
			<span style="color:red"><?php echo e($errors->first('product_id')); ?> </span>
		</td>
		<td><input type="text" id="unit" name="unit[]" readonly class="form-control unit" value="<?php echo e($key1->product->unit->name); ?>" placeholder="Unit" >
			<span style="color:red"><?php echo e($errors->first('unit')); ?> </span>
		</td>
		<td><input type="text" id="avilable_qty" name="avilable_qty[]" class="form-control avilable_qty" value="<?php echo e($key1->avilablepro); ?>" readonly placeholder="Avilable Qty">
			<input type="hidden" class="old_original"  value="<?php echo e($key1->avilablepro); ?>">
            <span style="color:red"><?php echo e($errors->first('avilable_qty')); ?> </span>
		</td>
		<td><input type="text" id="new_qty" name="quantity[]" class="form-control quantity" required value="<?php echo e($key1->qty); ?>" placeholder="Qty">
             <input type="hidden" class="old_quantity" value="<?php echo e($key1->qty); ?>">
             <input type="hidden" class="total_quantity" value="<?php echo e($key1->totalqty); ?>">
			 <input type="hidden" class="sale_qty" value="<?php echo e($key1->saleqty); ?>">
             
            <span style="color:red"><?php echo e($errors->first('quantity')); ?> </span>
		</td>
		
		<td>	
				<?php if(count($sale->items) == $va1+1): ?>
...
<?php echo $__env->make('layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u269893594/domains/easyachievers.in/public_html/universal/resources/views/admin/sales/edit.blade.php ENDPATH**/ ?>